...
doTest <- function(object1, object2,threadsNo,nbins,baseLevel){
	...
	intercept1 <- mcols(object1)$Intercept + mcols(object1)[[baseCondition]]
	intercept2 <- mcols(object2)$Intercept + mcols(object2)[[baseCondition]]
	dispersion1 <- dispersions(object1)
	dispersion2 <- dispersions(object2)
	log2Ratio1 <- results(object1)$log2FoldChange
	log2Ratio2 <- results(object2)$log2FoldChange
...
callDESeq2 <- function(data, condition, baseLevel){
	...
	dataSet <- suppressMessages(nbinomWaldTest(dataSet))
	dataSet
}
#' @export
